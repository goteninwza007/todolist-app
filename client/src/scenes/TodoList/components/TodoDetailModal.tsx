...
const TodoDetailModal: React.FC<TodoDetailModalProps> = ({ todo, onClose, onEdit }) => {
  ...
  return (
    <div className="flex flex-col gap-5">
      <h2 className="text-lg font-semibold text-[#1a1a2e] leading-snug">{todo.title}</h2>
      <div className="flex flex-wrap gap-2">
        <Badge label={priority.label} colorClass={priority.color} dotClass={priority.dot} />
        <Badge label={size} colorClass="bg-violet-50 text-violet-600" dotClass="bg-violet-500" />
      </div>
      {todo.description && (
        <div>
          <p className="text-xs font-medium text-slate-400 mb-1">Description</p>
          <p className="text-sm text-slate-600 leading-relaxed">{todo.description}</p>
        </div>
      )}
      <div>
